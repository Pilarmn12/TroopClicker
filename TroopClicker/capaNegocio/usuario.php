...
class Usuario
{
    ...
    public function setContraseña(string $contraseña): array
    {
        ...
        if (strlen($contraseña) < 6 || strlen($contraseña) > 16) {
            /** Almacena el error en al array de errores. */
            $error[] = 'La contraseña debe tener entre 6 y 16 caracteres';
        }
        /** Comprueba que al menos tiene una letra minuscula. */
        if (!preg_match('`[a-z]`', $contraseña)) {
            /** Almacena el error en al array de errores. */
            $error[] = "La contraseña debe tener al menos una letra minúscula";
        }
        /** Comprueba que al menos tiene una letra mayuscula. */
        if (!preg_match('`[A-Z]`', $contraseña)) {
            /** Almacena el error en al array de errores. */
            $error[] = "La contraseña debe tener al menos una letra mayúscula";
        }
        /** Comprueba que al menos tiene un valor numerico. */
        if (!preg_match('`[0-9]`', $contraseña)) {
            /** Almacena el error en al array de errores. */
            $error[] = "La contraseña debe tener al menos un valor numérico";
        }
        /** Comprueba si no hay errores. */
        if (!$error) {
            /** Inicializa el valor de la propiedad. */
     ...
     * @return array[]:string Array de errores.
     */
    public function setNombre(string $nombre): array
    {
        /** Inicializa la propiedad. */
        /** @var array[]:string  Array vacío, supone que no hay errores. */
        $error = array();

        if (!preg_match('`^[A-Z][a-z]{2,30}$`', $nombre)) {
            /** Inicializa el valor de la propiedad. */
            $error[] = 'El nombre debe empezar por mayuscula y contener solo letras';
        }
        /** Comprueba si no hay errores. */
        if (!$error) {
            /** Inicializa el valor de la propiedad. */
